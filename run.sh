...
SERVICES=(cognito api)
SERVICES_DIR=(cognito api)
...
function retrieveYarnCache() {
  ...
  echo "docker run --rm --entrypoint cat ${COMPOSE_PROJECT_NAME}_$SERVICE_NAME:latest /cache/yarn.lock > /tmp/${SERVICE_NAME}_yarn.lock"
  docker run --rm --entrypoint cat ${COMPOSE_PROJECT_NAME}_$SERVICE_NAME:latest yarn.lock > /tmp/${SERVICE_NAME}_yarn.lock
